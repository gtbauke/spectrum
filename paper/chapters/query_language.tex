...
SELECT [TOP <n> | PARETO] <campos>
FROM [<nome do modelo ou identificador>]
[WHERE <condicoes>]
...
\begin{enumerate}
    ...
    \begin{lstlisting}[language=Sql]
    SELECT TOP 10 expression, fitness, size FROM my_model PATTERN IS LIKE "sin(v0)"
    \end{lstlisting}

    \item \textbf{Busca por interações específicas entre variáveis}:
    \begin{lstlisting}[language=Sql]
    SELECT PARETO expression, size FROM my_model WHERE PATTERN IS LIKE "x1 * x2"
    \end{lstlisting}
\end{enumerate}
