...
from app.core.ports.repositories.immutable import IImmutableRepository
...
from app.features.profiles.blocks.inference.domain.where import (
...
class IInferenceRunRepository(IVersionedRepository[InferenceRun, InferenceRunWhere, InferenceRunFilter], Protocol):
    async def update(self, entity: InferenceRun) -> None: ...


class IInferenceResultRepository(IImmutableRepository[InferenceResult, InferenceResultWhere, InferenceResultFilter], Protocol):
...
class SqlAlchemyInferenceRunRepository(
    ...
    async def get_unique(self, where: InferenceRunWhere) -> InferenceRun | None:
        query = select(
            ...
            else self._model_class.is_latest.is_(True)
        )
    ...
    async def get_latest_version(self, parent_id: UUID) -> InferenceRun | None:
        ...
        query = select(self._model_class).where(
            ...
            self._model_class.is_latest.is_(True)
        )
        result = await self._session.execute(query)
        orm = result.scalars().first()
        return self._mapper.to_domain(orm) if orm else None
    ...
    async def unset_latest_and_add(self, parent_id: UUID, new_entity: InferenceRun) -> None:
        ...
        await self._session.execute(
            ...
            .where(self._model_class.block_id == parent_id, self._model_class.is_latest.is_(True))
