...
class PredictionEvaluationService:
    ...
    def evaluate_expression(
        ...
        # Prepare a 2D Feature Matrix 'x' if variables are simple feature columns (x0, x1)
        # This mirrors ValidationService logic for vector ops like x[:, i]
        feature_cols = [v for k, v in variables.items() if not k.startswith("t") and hasattr(v, "__len__")]
        if feature_cols:
            try:
                eval_context["x"] = np.column_stack(feature_cols)
            except Exception:
                pass


        # Add parameters t0, t1, etc.
        p_values = []
        ...
        try:
            ...
            predicted_y = np.nan_to_num(predicted_y, nan=0.0, posinf=1e9, neginf=-1e9)
            return predicted_y
