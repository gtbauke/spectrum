...
export const profileSchema = rawProfileSchema.transform((data) => ({
	...
	mode: data.mode,
	datasets: data.datasets.map((d: any) => d.ownerId ? d : datasetSchema.parse(d)),
	jobs: data.jobs.map((j: any) => j.profileId ? j : jobSchema.parse(j)),
	models: data.models.map((m: any) => m.profileId ? m : modelSchema.parse(m)),
	blocks: data.blocks.map((b: any) => b.kind ? b : blockSchema.parse(b)),
