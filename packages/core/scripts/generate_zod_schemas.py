...
IGNORED_BASES = {'RootDomainModel', 'BaseMutableDomainModel', 'BaseImmutableDomainModel', 'BaseImmutableVersionedDomainModel'}
...
def map_type_to_zod(annotation: Any, required: bool, deps_collected: Set[str], field_name: str) -> str:
    ...
    ALWAYS_REQUIRED_FIELDS = {'id', 'created_at', 'updated_at', 'timestamp', 'version', 'is_latest'}
    ...
    if origin is list or type_name == 'list':
        if args:
            inner_type = map_type_to_zod(args[0], required=True, deps_collected=deps_collected, field_name=field_name)
...
def generate(target_model_name: str | None = None):
    ...
    if target_model_name:
        ...
        print(f"Discovered {len(collected_models)} models in the dependency tree for '{target_model_name}':")
    else:
        modules = get_all_modules()
        for module in modules:
            for name, obj in inspect.getmembers(module, inspect.isclass):
        ...
        print(f"Discovered {len(collected_models)} models across all domain objects:")

    for m in collected_models:
        print(f" - {m.__name__}")
        
    frontend_dir = scripts_dir.parent.parent / "frontend" / "app" / "schemas" / "generated"
    ...
    for model in collected_models:
        ...
        if not is_enum:
            for field_name, field_info in model.model_fields.items():
                ...
                zod_type_str = map_type_to_zod(field_info.annotation, required=required, deps_collected=deps_collected, field_name=field_name)
        ...
        with open(out_path, 'w') as f:
            ...
            for dep in deps_collected:
                if dep != model.__name__: # avoid self-import in case of direct cyclic link
                    ...
                    f.write(f'import {{ {dep_schema_name} }} from "./{dep_file}";\n')
            ...
            if is_enum:
                ...
                f.write(f'export const {camel_model_name}Values = [{", ".join(enum_values)}] as const;\n')
                f.write(f'export const {camel_model_name}Schema = z.enum({camel_model_name}Values);\n\n')
            else:
                f.write(f'export const {camel_model_name}Schema = z.object({{\n')
                f.write('\n'.join(fields_ts))
                f.write('\n});\n\n')
            
            f.write(f'export type {model.__name__} = z.infer<typeof {camel_model_name}Schema>;\n')
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate Zod schemas from Pydantic domain models.")
    parser.add_argument("model_name", nargs="?", default=None, help="Specific model name to generate (e.g. Dataset)")
    parser.add_argument("--file", help="Path to a specific python file to generate schemas for models defined within it")
    ...
    if args.file:
        ...
        try:
            ...
            for name, obj in inspect.getmembers(module, inspect.isclass):
                ...
                if obj.__module__ == module_path:
                    if (issubclass(obj, RootDomainModel) and obj.__name__ not in IGNORED_BASES) or issubclass(obj, enum.Enum):
                        models_found = True
                        print(f"Found model/enum '{obj.__name__}' in file, generating schemas...")
            ...
            print(f"Error: The file {args.file} is not inside the core package directory.")
            sys.exit(1)
        except Exception as e:
