...
class S3FileStorage(FileStorage):
    def __init__(
        ...
        try:
            ...
            logger.error("aioboto3 is not installed. S3FileStorage will not function.")
            raise ImportError("aioboto3 is required for S3FileStorage. Install it in the host package (backend/workers).")
    ...
    async def upload(self, *, path: str, file: BinaryIO) -> UploadResult:
        ...
        async with session.client("s3", endpoint_url=self._endpoint_url) as s3:
            ...
            checksum = response.get("ETag", "").strip('"')  # ETag is often the MD5
