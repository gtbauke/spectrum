...
class InferenceRunRequestedEvent(BaseModel):
    ...
    block_id: UUID = Field(..., description="The ID of the block where the query is from")
    profile_id: UUID = Field(..., description="The ID of the profile owning the block")
    query: str = Field(..., description="The IQL query to execute")

    @property
    def routing_key(self) -> str:
        return "inference.requested"
