...
Cruzamento é o processo de combinação dos genomas de dois indivíduos para geração de novos indivíduos~\cite{galr}. Normalmente, o cruzamento é realizado escolhendo-se um ponto aleatório de corte na representação genética dos indivíduos e trocando-se as partes dos genomas a partir desse ponto.
...
\begin{enumerate}
  ...
  \item Repita os passos anteriores até que nenhuma nova equivalência seja
\end{enumerate}
...
\subsection{Algoritmo: Eggp}

Eggp (E-graph Genetic Programming) é uma implementação de um algoritmo genético para regressão simbólica que utiliza e-graphs e saturação de igualidade~\cite{eggp}. O algoritmo segue os passos tradicionais de um algoritmo genético, mas com algumas diferenças importantes:
\begin{enumerate}
...
\section{Outros métodos para Regressão Simbólica}

Apesar de algoritmos genéticos serem os mais comuns para implementação de regressão simbólica, outros métodos também podem ser utilizados~\cite{Bartlett_2024, Makke2024}. Outros métodos incluem:
...
\subsection{Interação-Transformação}

A regressão simbólica utilizando da estrutura de Interação-Transformação (IT) é uma abordagem que restringe o espaço de busca de expressões matemáticas para aquelas que podem ser representadas como uma combinação polinomial das variáveis seguidas por uma transformação nã-linear~\cite{lsritr}. Diferentemente dos algoritmos genéticos, nessa implementação, algumas relações não podem ser representadas.

% TODO: add more information about Interaction-Transformation symbolic regression.

\subsection{Regressão Simbólica Exaustiva}

\section{Aplicações da Regressão Simbólica}

\section{Ferramentas para Regressão Simbólica}

\subsection{HeuristicLab}

HeuristicLab é uma ferramenta gráfica que, dentre outros algoritmos, suporta a exploração de algoritmos de regressão simbólica.

% TODO: add more information about HeuristicLab.

\subsection{TuringBot}

Assim como a ferramenta anterior, TuringBot é uma interface gráfica para exploração de algoritmos de regressão simbólica. Ele mostra ao usuário as melhores expressões encontradas para o conjunto de dados fornecido com algumas métricas simples do modelo criado~\cite{rEGGression}.
